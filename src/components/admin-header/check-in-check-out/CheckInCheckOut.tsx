...
import { useCreateCheckInMutation, useCreateCheckOutMutation } from "@/services";
...
export function CheckInCheckOut() {
  ...
  // Check In Mutation
  const { onCheckInMutation, isPending: isCheckInPending } = useCreateCheckInMutation({
  ...
  // Check Out Mutation
  const { onCheckOutMutation, isPending: isCheckOutPending } = useCreateCheckOutMutation({
...
const handleButtonClick = async () => {
  ...
  if (!isCheckedIn) {
    // Use type assertion if you're confident about the API
    onCheckInMutation({
      staffId,
      inTime: currentTime,
      date: currentDate,
