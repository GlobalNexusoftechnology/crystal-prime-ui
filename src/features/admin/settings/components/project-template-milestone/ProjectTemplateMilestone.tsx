...
export function ProjectTemplateMilestone({
  formik,
  readOnly = false,
}: {
  ...
  const [openMilestones, setOpenMilestones] = useState<{
    ...
    if (values.milestones && values.milestones.length > 0) {
      ...
      return Object.fromEntries(
        values?.milestones?.map((m: Milestone, idx: number) => [
          m.id || String(idx),
          true,
        ]),
      );
                ...
                <MilestoneRow
                  ...
                  onSave={() => {
                    ...
                    const firstTask =
                      ...
                      currentMilestone.tasks[0];
                    if (firstTask) {
                      setEditingTaskId(
                        (firstTask.id ? firstTask.id : String(0)) +
                          "-" +
                          Date.now(),
                      );
                    } else {
