...
interface TaskProps {
    ...
    setMenuOpen: (menu: number | null) => void;
            ...
            {editingTask === task.id && editTask ? (
                ...
                <>
                    ...
                    <td className="py-2 text-right relative">
                        <button className="text-gray-400 hover:text-blue-600" title="Menu" type="button" onClick={() => setMenuOpen(menuOpen === task.id ? null : task.id)}><HiOutlineDotsVertical className="w-6 h-6" /></button>
                        {menuOpen === task.id && (
                            <div className="absolute right-0 mt-2 bg-white border rounded shadow z-10 min-w-[100px]">
                                <button className="block w-full text-left px-4 py-2 hover:bg-gray-100" onClick={() => onEdit(task)}>Edit</button>
                                <button className="block w-full text-left px-4 py-2 hover:bg-gray-100 text-red-600" onClick={() => onDelete(task.id)}>Delete</button>
                            </div>
                        )}
                    </td>
                </>
