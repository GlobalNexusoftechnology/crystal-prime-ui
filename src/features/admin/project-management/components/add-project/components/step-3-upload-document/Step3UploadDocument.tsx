...
export function Step3UploadDocument({ onBack, onNext }: Step3UploadDocumentProps) {
  ...
  const validateFiles = (fileList: FileList | File[], existingFiles: File[]): { validFiles: File[], errorMsg: string } => {
    const validTypes = [
    ...
    for (let i = 0; i < fileList.length; i++) {
      ...
      if (existingFiles.some(ef => ef.name === f.name && ef.size === f.size)) {
        continue;
      }
      validFiles.push(f);
    }
    ...
    <div className="flex flex-col gap-8 2xl:gap-[2vw]">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="font-semibold text-lg 2xl:text-[1.2vw] text-gray-700">Upload Document(s)</span>
          <span className="text-sm text-red-400">Supported: XLS, XLSX – Max 2MB each</span>
          {files.length > 0 && (
            <div className="flex flex-col mt-2 gap-1">
              {files.map((f, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <span className="text-green-600">{f.name}</span>
                  <button
                    type="button"
