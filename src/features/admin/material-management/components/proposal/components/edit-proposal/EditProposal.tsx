...
export function EditProposal() {
    ...
    if (
      ...
      if (d.items && d.items.length > 0) {
        ...
        const materialsFromItems = d.items.map((item) => {
          ...
          const originalMaterial = allMaterialsData?.data?.find((m) => m.id === item.materialId);
          return {
            id: item.materialId,
  ...
  React.useEffect(() => {
    if (selectedMaterials.length > 0) {
      ...
      const metalMaterials = selectedMaterials.filter(material => 
        material.type && isMetalMaterial(material.type)
      );
      ...
      if (metalMaterials.length > 0) {
        ...
        const mostCommonMetalDiscount = metalDiscounts.reduce((a, b) => 
          metalDiscounts.filter(v => v === a).length >= metalDiscounts.filter(v => v === b).length ? a : b
        );
        setMetalDiscount(mostCommonMetalDiscount);
      }

      // Find the most common discount value for plastic materials
      const plasticMaterials = selectedMaterials.filter(material => 
        material.type && isPlasticMaterial(material.type)
      );
      ...
      if (plasticMaterials.length > 0) {
        ...
        const mostCommonPlasticDiscount = plasticDiscounts.reduce((a, b) => 
          plasticDiscounts.filter(v => v === a).length >= plasticDiscounts.filter(v => v === b).length ? a : b
        );
  ...
  const isMetalMaterial = (materialType: string): boolean => {
    ...
    const isMetal = materialTypeLower.includes("metal") || 
  ...
  const isPlasticMaterial = (materialType: string): boolean => {
    const materialTypeLower = materialType.toLowerCase();
    const isPlastic = materialTypeLower.includes("plastic") || 
