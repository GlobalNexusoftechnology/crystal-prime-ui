import React from "react";

type TFacebookIconProps = {
  width?: number; // Width of the icon
  height?: number; // Height of the icon
  fill?: string;
  background?: string; // Fill color of the icon
};

/**
 * ---------------------------------------------------------------------------
 * A Facebook icon with dynamic width, height, and fill color.
 */
export function FacebookIcon({
  width = 24,
  height = 24,
  fill = "white",
  background = "#1dc4c4",
}: TFacebookIconProps) {
  return (
    <svg
      width={width}
      height={height}
      viewBox="0 0 18 18"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      {/* <rect width="18" height="18" fill={background} rx="6" /> */}
      <circle cx="9" cy="9" r="9" fill={background} />

      <g clipPath="url(#clip0_432_34)">
        <path
          d="M11.9978 2.98875H13.641V0.12675C13.3575 0.08775 12.3825 0 11.247 0C8.87776 0 7.25476 1.49025 7.25476 4.22925V6.75H4.64026V9.9495H7.25476V18H10.4603V9.95025H12.969L13.3673 6.75075H10.4595V4.5465C10.4603 3.62175 10.7093 2.98875 11.9978 2.98875Z"
          fill={fill}
        />
      </g>
      <defs>
        <clipPath id="clip0_432_34">
          <rect width="18" height="18" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
}
