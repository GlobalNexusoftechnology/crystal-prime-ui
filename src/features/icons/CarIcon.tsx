import React from "react";

type TInstagramIconProps = {
  width?: number;
  height?: number;
  fill?: string;
};

/**
 * ---------------------------------------------------------------------------
 * A custom Instagram-like icon with multiple paths for the design.
 */
export function CarIcon({
    width = 48,
    height = 48,
//   fill = "white",
}: TInstagramIconProps) {
  return (
    <svg
    width={width}
    height={height}
    viewBox="0 0 48 48"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
 
  >
    <path
      d="M4 14C4 9.58172 7.58172 6 12 6H20C24.4183 6 28 9.58172 28 14V35C28 36.6569 26.6569 38 25 38H12C7.58172 38 4 34.4183 4 30V14Z"
      fill="#EAB4FE"
    />
    <path
      d="M18 38C18 40.2091 16.2091 42 14 42C11.7909 42 10 40.2091 10 38C10 37.916 10.0026 37.8326 10.0077 37.7499C10.1368 35.6573 11.8748 34 14 34C16.2091 34 18 35.7909 18 38Z"
      fill="white"
    />
    <path
      d="M40 38C40 40.2091 38.2091 42 36 42C33.7909 42 32 40.2091 32 38C32 35.7909 33.7909 34 36 34C38.2091 34 40 35.7909 40 38Z"
      fill="white"
    />
    <path
      d="M28 38V14M28 38H32M28 38H18M28 14C28 9.58172 24.4183 6 20 6H12C7.58172 6 4 9.58172 4 14V30C4 33.7304 6.55333 36.8645 10.0077 37.7499M28 14H34.4182C35.4344 14 36.4126 14.3868 37.154 15.0819L42.7358 20.3148C43.5424 21.071 44 22.1273 44 23.2329V34C44 36.2091 42.2091 38 40 38M40 38C40 40.2091 38.2091 42 36 42C33.7909 42 32 40.2091 32 38M40 38C40 35.7909 38.2091 34 36 34C33.7909 34 32 35.7909 32 38M18 38C18 40.2091 16.2091 42 14 42C11.7909 42 10 40.2091 10 38C10 37.916 10.0026 37.8326 10.0077 37.7499M18 38C18 35.7909 16.2091 34 14 34C11.8748 34 10.1368 35.6573 10.0077 37.7499"
      stroke="#141718"
      strokeWidth={2.5}
    />
    <path
      d="M20 16H16"
      stroke="#141718"
      strokeWidth={2.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M20 24H12"
      stroke="#141718"
      strokeWidth={2.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
  );
}